#!/bin/bash
# One-command install: skills symlinks + SessionStart hook + smoke test.
...
repo = sys.argv[1]
entry = {"type": "command", "command": f"bash {repo}/hooks/session-start.sh",
...
if ours:
    ours[0].update(entry)
    print(f"✓ hook already present — path refreshed in {p}")
else:
    groups.append({"hooks": [entry]})
    print(f"✓ hook registered in {p}")
